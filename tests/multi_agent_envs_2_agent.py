...
def create_multiagent_env():
    ...
    #     reload_list.append(item)
    e = SynchronousMultiAgentEnv(num_agents=num_agents, grid_size=[gridsize, gridsize], capacities=[40 for _ in range(num_agents)], reloads=reload_list,
    ...
    for item in init_state:
        ...
        for item2 in targets:
            ...
            if not item==item2:
                ...
                #result=np.random.uniform(0,0.0)
...
def create_costs_for_agents_targets(consmdp,agent_lists,cost_lists,init_state,agent_target):
    ...
    for i in range(len(init_state)):
        Bottleneckgraph.add_node(init_state[i],bipartite=0)

    for j in range(len(agent_lists)):
        Bottleneckgraph.add_node(-j-1,bipartite=1)
    ...
    print(dict_costs,nx.bipartite.is_bipartite(Bottleneckgraph))
...
if __name__ == "__main__":
    ...
    final_out_dict=[]
    final_paths=[]
    Graph_cost, agent_lists2, cost_lists2, paths_save2 = multi_agent_codes.tarjan_scc_init(Graph_cost, num_agent, T)
    bottleneck_graph2, dict_in2, dict_out2 = create_costs_for_agents_targets(MDP, agent_lists2, cost_lists2, init_state,
                                                                             graph_agent_target_cost)
    # allocates targets to agents
    # print("Computing bottleneck assignment")
    ...
    final_assignments2, final_costs2 = multi_agent_codes.augment_matching(matching2, agent_lists2, init_state,
                                                                          bottleneck_graph2)
    ...
    MDP = consmdp1[0]
    #reload_list=list(random.sample(range(gridsize*gridsize), 10))
    ...
    #     reload_list.append(item)
    print(reload_list)
    env = SynchronousMultiAgentEnv(num_agents=num_agents, grid_size=[gridsize, gridsize], capacities=[40 for _ in range(num_agents)], reloads=reload_list,
                                 targets=targets, init_states=init_state,
